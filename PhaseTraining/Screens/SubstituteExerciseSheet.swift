        ...
        HStack(alignment: .top, spacing: 8) {
            Button {
                ...
                HStack(alignment: .top, spacing: 12) {
                    ExerciseThumbnail(exerciseID: sub.exercise.id, urlString: sub.exercise.thumbnailURL ?? sub.exercise.imageURL, size: ExerciseTile.thumbSize, cornerRadius: 12)
    ...
    private func fallbackRow(_ ex: Exercise) -> some View {
        HStack(alignment: .top, spacing: 8) {
            Button {
                ...
                HStack(alignment: .top, spacing: 12) {
                    ExerciseThumbnail(exerciseID: ex.id, urlString: ex.thumbnailURL ?? ex.imageURL, size: ExerciseTile.thumbSize, cornerRadius: 12)
                    VStack(alignment: .leading, spacing: 8) {
                        Text(ex.name)
                            .styled(.body)
                            .foregroundStyle(Color.ink)
