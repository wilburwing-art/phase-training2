...
final class ProfileFieldCoverageTests: XCTestCase {
    ...
    private static let probes: [Probe] = [
        ...
        Probe(name: "exerciseAffinities", mutateForHash: nil, mutateForSnapshot: nil,
              snapshotMarker: nil,
              skipReason: "Captured by the post-workout action sheet (bumpAffinity) and by exercise swaps (MemoryStore.recordSwap). The season engine consumes it via AthleteState.exerciseAffinities as a SOFT comparator key (A1, 2026-09-25) — kept OUT of planInputsHash so a swap biases the next generation without silently rebuilding the current week. Coach context doesn't surface raw scores."),
        Probe(name: "suggestionDecisions", mutateForHash: nil, mutateForSnapshot: nil,
              snapshotMarker: nil,
              skipReason: "A4 bookkeeping: which weekly-check-in suggestions the user accepted or dismissed, read only by PatternEngine to suppress repeats. The accepted ACTION (sessionMinutes, an exerciseAffinities sink, a saved routine) is what changes the plan, through its own field. The coach sees open suggestions, never this log."),
