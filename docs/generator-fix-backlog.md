...
- [x] **T0.1 — Accessory layer bypasses readiness scaling, deload, soreness exclusion, dislike filter, and the duration budget.** **DONE** (commit `aaf8cb9`): threaded `context.recentSoreAreas` + `excludeKws` + readiness×deload `setsMultiplier` into the append fns; budget-gated the append; 4 tests pin each bypass. ONE root cause, multiple symptoms.
...
- [x] **T1.1 — Bodyweight pull day collapses to ~1 exercise.** **DONE** (surgical, prune preserved). The catalog has no apparatus-free vertical/horizontal pull (physically accurate). Fix, all SURGICAL into the shipped **pruned 564 base** (a full `build_db.py` rebuild would undo the deliberate niche-exercise prune — 564 vs stale source 755; see [[phase-training-coachdb-commit-vs-artifact-drift]]): (1) added Table Inverted Row + Towel Door Row (apparatus-free horizontal pull — catalog had none); (2) restored 6 bodyweight back/scapular activations (Prone Snow Angel, Scapular Retraction Prone W, Prone T Raise, Scapular Wall Slide, Scapular Push-Up, Prone Y-T-W-L Complex) — NOT the prehab/mobility/balance firehose; (3) degradation floor (T1.3) backfills the empty vertical-pull slot. Bodyweight pull day: 1 → ≥3 movements. Full unit suite green; firehose stays pruned (balance/breathing/recovery/coordination = 0).
- [x] **T1.2 — Time-based movements prescribed in reps.** **DONE** (commit `b9c4527`): the root cause was `focusBias`/era clobbering the time/distance string, NOT a missing `default_duration` read — 71 exercises store time ("30-60 sec"), distance ("40-60 ft"), or holds in `default_reps`. Now preserved over the bias/era band via `isNumericRepBand`. ⚠ 5 exercises (Tuck Hold et al.) store time only in `default_duration` with null `default_reps` — still unreached; needs the `Exercise` model to load `default_duration`. Follow-up. Prescription ignores coach.db `default_duration` ("30-60 sec") and synthesizes a rep band when `default_reps` is null (Tuck Hold → `3×10-12`; Reverse Plank / Wall Sit / Battle Ropes / Farmer Carry → "3×8-12"). Fix: emit a **time** prescription when `default_reps` is null and `default_duration` is set. Locus: `WorkoutGenerator.swift:~1080-1081` + `defaultRepsFromFormula` `:1214-1217`.
- [x] **T1.3 — No graceful-degradation floor when equipment empties slots.** **DONE** (commit `8275d09`, with T1.1b): a post-pass floor backfills below a 3-movement minimum from focus-appropriate fallback patterns through the same `pickForSlot` + `makePickedRow` pipeline (extracted to avoid the dual-path trap), budget-gated. General — not just the pull day.
- [x] **T1.4 — Duration budget is dead upward.** **DONE** (branch `generator-backlog-batch`). Added a two-tier high-minute volume ramp in `WorkoutGenerator.generateLift`: at ≥90 min, one additional focus-appropriate accessory slot is attempted via `highMinuteAccessoryPatterns(for:)` + `pickForSlot` + `makePickedRow` (same pipeline as the degradation floor, budget-gated); at ≥120 min, every compound lift in the session earns +1 set (capped at 6), with `elapsedSec` updated so `estimatedMinutes` stays accurate. `test_highMinuteBudget_addsVolumeOver60min` asserts a 120-min day has strictly more total sets than a 60-min day for identical inputs.
- [x] **T1.5 — Beginner gets unmodulated near-max work.** **DONE** (commit `ec361b5`): beginners now capped to RPE ≤7 (extracted `capRPE`, composes with the readiness cap) and numeric rep bands <6 floor to 6-8. ⚠ DEFAULT CHOICE flagged: floor 6 also nudges canonical novice 5×5 → 6-8; drop to 5 in `beginnerRepFloor` to preserve 5×5.
- [x] **T1.6 — Advanced ≡ intermediate dose.** **DONE** (branch `generator-backlog-batch`). Advanced compounds now step +1 over the intermediate clamp (`min(sets,4)+1`), topping out at 5 — the canonical 5×5 stays 5 (general-strength raw 5 → `min(5,4)+1`), while raw-4 focuses (hypertrophy) step 4 → 5. Isolations keep the raw bias. The old `break` was a no-op whenever the raw bias was ≤4 (every focus except general strength, which already differentiated). Tested `test_prescription_advancedAddsCompoundSetOverIntermediate`; 5×5 regression guard (`test_prescription_generalStrengthAdvanced_runs5x5`) still green.
- [x] **T1.7 — Dislike filter is name-substring, not equipment-tag.** **DONE** (commit `5a591f1`): the exclude filter in `CoachDatabase.exercises()` and the accessory `pickAccessoryByName` now match dislikes against each required equipment's slug AND display name (new `equipmentNameBySlug()`), catching slug-encoded machines (`leg-curl-machine`) and name-only ones (`pec-deck` → "Pec Deck Machine").

---

## Tier 2 — Half-finished feedback loops (verify, then act)

- [x] **T2.1 — priorBest / bodyweight progression.** **DONE** (branch `generator-backlog-batch`). Critique REFUTED (no "190 lb on push-ups"); the real gap was bodyweight getting **no** target because `buildPriorBest` skipped `weight==0`. Fix: a true bodyweight movement (`LoggedExercise.isBodyweight`, unit "bw") now records `PriorBest(weight: 0, reps: bestReps)`, and `progressiveOverloadHint` emits "target: N+1 reps" for `weight==0` priors. Gated on `isBodyweight` so a 0 logged on a *loaded* lift is still ignored (preserves `test_priorBest_skipsZeroWeightOrUndoneSets`). `GeneratorContext.swift` builder + `WorkoutGenerator.swift` hint; tested builder + hint paths.
- [x] **T2.2 — Age 55→70 undifferentiated; no power/balance bias by 70.** **DONE** (branch `generator-backlog-batch`). Added a new `EraCohort.veteranStrength` ("veteran_strength") case for age ≥70 with a low/mid rep-range bias (compounds 3-6 for neural drive/power, isolations 8-12), barbell/DB aesthetic, and upper-lower split preference. `EraAffinity.derivedCohort` now returns `.magazineBodybuilding` for 55-69 and `.veteranStrength` for 70+. `DemographicProfile.from` handles 70+ with ageDelta=-2, sessHigh capped at 45 min, and minRecoveryDaysBetweenLifts=2. `WorkoutGenerator.prescription` applies a second set reduction for age ≥70 on top of the 55+ one. `ReadinessSignal.cohortNorm` maps `.veteranStrength` to 2.5. Existing era tests updated for the new cohort count (6) and split boundary; `test_age70_differentiatesFrom55` asserts lower session cap and more recovery days vs 55+.
