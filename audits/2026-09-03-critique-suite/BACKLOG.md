...
- [ ] 23 prose em/en dashes in user-facing `Text()` literals, against the house ban. Two more are numeric ranges and are a separate call. *(08 F4)*
- [ ] `OnboardingWelcomeScreen.swift:27` says "Eight quick questions"; `OnboardingStep.total` is 10 and the flow has 11 screens. Carried unfixed from the previous cycle's Tier 4. *(12)*
- [ ] `CoachConsent.swift:4-5` says the coach is "on by default for new installs"; T0-5 made it an affirmative choice with neither option pre-selected. *(11 F5)*
- [ ] `docs/privacy.md:23` says Health-confirmed sessions may reach the Coach context. They do not; greps for health and body-composition symbols in `PhaseTraining/Coach/` are empty. *(11 F4)*
