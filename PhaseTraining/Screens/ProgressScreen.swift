// ProgressScreen.swift — Phase 12 Progress tab.
//
// Three sections:
//   1. Sessions per week (last 8 weeks) — bar chart, target line at
//      memory.liftDaysPerWeek + sport days.
//   2. Volume trend (last 8 weeks) — total weight × reps × sets, line chart.
//      Falls back to "set count" trend for users who log without weights.
//   3. Recent feedback — latest 10 FeedbackEntry rows, newest first, with
//      difficulty chip + hurt-area summary.
//
// Empty state when SessionStore has no saved sessions. No SwiftUI Charts
// dependency — hand-drawn bars + line keep the bundle stable on iOS 17.
...
#Preview {
    ...
    // Seed 5 sessions across the last 4 weeks.
    let cal = Calendar.current
    let now = Date()
    var sessions: [SavedSession] = []
    for daysBack in [1, 4, 9, 12, 22] {
        ...
        sessions.append(SavedSession(
            ...
            exercises: [
                LoggedExercise(
                    ...
                    sets: [
                        LoggedSet(num: 1, weight: "135", reps: "5", rpe: "7", done: true),
                        LoggedSet(num: 2, weight: "145", reps: "5", rpe: "8", done: true),
                        LoggedSet(num: 3, weight: "155", reps: "5", rpe: "9", done: true),
                    ],
                    prevSets: []
                )
            ],
